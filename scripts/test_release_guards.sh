...
grep -Fq -- '(.assets | length == 24)' "${release_workflow}" ||
  die "remote draft validation must require exactly 24 assets"
...
[[ ! -e "${asset_dir}/mem-checksums.txt" ]] ||
  die "empty asset directory must not produce a server manifest"
assets=(
  memd-darwin-amd64
  memd-darwin-arm64
  memd-linux-amd64
