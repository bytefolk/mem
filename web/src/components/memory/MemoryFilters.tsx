  ...
  return (
    ...
    >
      ...
      <label className="grid gap-1">
        <span className="text-left text-2xs uppercase tracking-wider text-fg-subtle">{t('memory.kind')}</span>
        <select
          value={value.kind ?? ''}
          onChange={(event) =>
            update({ kind: (event.target.value || undefined) as MemoryKind | undefined })
          }
          className="h-9 rounded-md border border-border bg-bg-inset px-2.5 text-left [text-align-last:left] text-xs text-fg outline-none focus:border-accent/60"
      ...
      <div className="grid gap-1">
        <span className="inline-flex items-center justify-start gap-1 text-left text-2xs uppercase tracking-wider text-fg-subtle">
      ...
      <div className="flex items-end gap-2">
        <label className="grid min-w-28 flex-1 gap-1">
          <span className="text-left text-2xs uppercase tracking-wider text-fg-subtle">
            {t('memory.pinFilter')}
          </span>
          <select
