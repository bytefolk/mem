...
func saveQoderCheckpointLocked(stateDir, p string, cp qoderCheckpoint) error {
	...
	if current.LastLine > cp.LastLine {
		// A delayed writer must not replace a newer cursor's diagnostic state
		// while preserving only its line number. The newer successful checkpoint
		// remains the complete source of truth until a writer advances it.
		cp = current
	}
