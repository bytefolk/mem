...
func TestQoderCheckpointHelperProcess(t *testing.T) {
	...
	case "save-hold-lock":
		...
		// Deliberately do not release lock: os.Exit below models an unexpected
		// process exit and lets the parent verify OS-backed recovery.
		heldLock = lock
	...
	// Keep the descriptor live through the release wait. This is a real
	// process-held lock, not merely a lock acquired before an early finalizer.
...
func TestSaveQoderCheckpointKeepsHighestLastLineAcrossIndependentProcesses(t *testing.T) {
	...
	// The old writer used a shared staging path and blindly replaced the cursor:
	// this deterministic high-then-low interleaving ended at 4. These are
	// independent live processes (the high writer still waits for release), and
	// the checkpoint must retain the greatest successfully submitted cursor.
...
func TestSaveQoderCheckpointSerializesAndRecoversAfterOwnerExit(t *testing.T) {
	...
	// The high helper exits via os.Exit without closing its descriptor. The OS
	// must release the advisory lock, allowing the already-waiting low process
	// to proceed without a stale-lock cleanup path or a shared staging collision.
	writeQoderCheckpointRelease(t, highRelease)
	high.wait(t)
	waitForQoderCheckpointSignal(t, lowReady)
	low.wait(t)
