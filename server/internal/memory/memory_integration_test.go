...
func TestMemoryPostgres(t *testing.T) {
	...
	t.Run("relations encode immutable supersede and correct chains", func(t *testing.T) {
		...
		if err != nil || len(outboundResult.Relations) != 1 ||
			...
			t.Fatalf("outbound relations = %+v err=%v", outboundResult.Relations, err)
		}
		...
		if err != nil || len(inboundResult.Relations) != 1 || inboundResult.Relations[0].SourceID != fresh.Memory.ID {
			t.Fatalf("inbound relations = %+v err=%v", inboundResult.Relations, err)
		}
		...
		if err != nil || len(typedResult.Relations) != 0 {
			t.Fatalf("type-filtered relations = %+v err=%v", typedResult.Relations, err)
		}
