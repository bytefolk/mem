...
const hnswCorpusUser = "00000000-0000-0000-0000-000000000173"

// Populated 24→25→24→25, ingest after index build, wrong-dimension rejection,
// and EXPLAIN of the shipping cosine-order text/visual shapes.
func TestHNSWMigrationPostgres(t *testing.T) {
	...
	exec(`INSERT INTO users(id,email,password_hash)
		VALUES ('` + hnswCorpusUser + `','hnsw@example.test','test');
		INSERT INTO files(id,user_id,name,path,size,sha256,mime,storage_key)
		SELECT md5(i::text)::uuid,'` + hnswCorpusUser + `',
		  'fixture-' || i, '/hnsw', 0, 'sha-' || i, 'text/plain', 'fixture-' || i
	...
	exec(`INSERT INTO files(id,user_id,name,path,size,sha256,mime,storage_key)
		VALUES (md5('2001')::uuid,'` + hnswCorpusUser + `',
		'after-index','/hnsw',0,'sha-2001','text/plain','after-index')`)
	for _, kind := range []string{"text", "visual", "face"} {
		dim, extraCols, extraValues := 512, "", ""
		if kind == "text" {
		...
		if err == nil || !strings.Contains(strings.ToLower(err.Error()), "dimension") {
	...
	t.Log("PASS: populated 24->25->24->25, valid cosine HNSW, ingest, dimension rejection, text/visual EXPLAIN uses HNSW")
}

func assertIndexScan(t *testing.T, ctx context.Context, db *sql.DB, index, query string) {
	t.Helper()
	rows, err := db.QueryContext(ctx, "EXPLAIN (ANALYZE, BUFFERS) "+query)
	if err != nil {
	...
	if !strings.Contains(text, index) {
		t.Fatalf("shipping plan did not use %s:\n%s", index, text)
	}
}
