...
// A chunk candidate budget must not become a file result budget or bypass
// owner/path/MIME/time scope. One file holding 101 nearest chunks must still
// yield k distinct eligible files, using HNSW continuation plus exact fallback.
func TestTextANNFileSemanticsPostgres(t *testing.T) {
	...
	addFile := func(user uuid.UUID, path, mime string, at time.Time, chunks int, far bool) uuid.UUID {
		...
		_, err := database.Pool.Exec(ctx, `INSERT INTO files(id,user_id,name,path,size,sha256,mime,storage_key,created_at,timeline_at)
			VALUES($1,$2,'fixture',$3,0,$4,$5,$6,$7,$7)`, id, user, path, id.String(), mime, id.String(), at)
		if err != nil {
			t.Fatal(err)
		}
