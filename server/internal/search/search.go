...
// three-tier shape as memory Recall (name substring → FTS → trigram) so a
...
func (s *Service) searchLexical(ctx context.Context, q Query, text string) ([]Hit, error) {
	// Search rejects empty queries before dispatch. Keep the numeric trigram
	// threshold here: a pg_trgm % prefilter uses a different similarity measure
	// and would discard valid word_similarity matches. This query scores the
	// filtered file set; index-backed candidate pruning is a separate change.
	args := []any{q.UserID}
	...
	sql := fmt.Sprintf(`
		WITH candidates AS (
			SELECT f.id AS file_id, f.name, f.path, f.mime, f.sha256,
			       f.summary, f.timeline_at, f.created_at,
			       strpos(lower(f.name), lower($%d)) > 0 AS name_contains,
		...
		ranked AS (
			SELECT candidates.*,
			       CASE
			           WHEN name_contains THEN 1.0::double precision
